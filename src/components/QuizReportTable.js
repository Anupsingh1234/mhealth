...
const AuditReport = (props) => {
  ...
  const [quiziddd, setQuiziid] = useState();
  const players = (e) => {
    setQuiziid(e.target.value);
  };
  ...
  const [getQuiz1, setGEtQuiz] = useState([]);
  //   console.log(getQuiz);
  const ques = (id) => {
  ...
  const [getQuizDetails, setQuizDetails] = useState([]);
  function submitQuiz(e) {
    e.preventDefault();
    axios
      .get(`${urlPrefix}v1.0/getUserQuizReport?quizId=${quiziddd}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          timeStamp: "timestamp",
          accept: "*/*",
      ...
      .then((res) => {
        ...
        {
          res.data.response.responseData
            ? setQuizDetails(res.data.response.responseData)
            : Message.error(res.data.response.responseMessage);
        }
      });
  }
        ...
        <div class="container" style={{ marginTop: "10px" }}>
          ...
          <form onSubmit={(e) => submitQuiz(e)}>
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <PrimaryButton mini className="w-[max-content]">
                {" "}
                Submit
              </PrimaryButton>
            </div>
          ...
          <div className="d-flex a-i-center">
            ...
            {getQuizDetails && getQuizDetails.length > 0 ? (
              <TablePagination
                ...
                component="div"
                count={getQuizDetails && getQuizDetails.length}
        ...
        >
          {getQuizDetails && getQuizDetails.length > 0 ? (
            ...
            >
              ...
              <TableBody style={{}}>
                {getQuizDetails &&
                  stableSort(getQuizDetails, getComparator(order, orderBy))
                      ...
                      return (
                        <>
                          <TableRow className="performace-table-row">
                            ...
                            <TableCell align="center" style={{ fontSize: 12 }}>
                              {" "}
                              {item.totalAttemptedQue
                                ? item.totalAttemptedQue
                                : "  -     "}
                            </TableCell>
                            <TableCell align="center" style={{ fontSize: 12 }}>
                              {" "}
                              {item.totalCorrectAns
                                ? item.totalCorrectAns
                                : "  -     "}
                            </TableCell>
                            {/* <TableCell align="center" style={{fontSize: 12}}>

...
export default function DataTable({
  ...
  const EditData = (row) => {
    ...
    //   : [],})
    setUser(
      Array.isArray(editData.labUserIds)
        ? editData.labUserIds?.map((item) => {
            if (editData.idwiseName.filter((elm) => elm.id == item)[0]) {
              return editData.idwiseName.filter((elm) => elm.id == item)[0][
                "userName"
  ...
  const IncDec = (item, plus) => {
    if (plus == true) {
      timeRange[
        timeRange.map((x, i) => [i, x]).filter((x) => x[1] == item)[0][0]
      ] = { [Object.keys(item)]: item[Object.keys(item)[0]] + 1 };
  ...
  const EditCapacity = () => {
    ...
    setAddCapacityData({
      ...
      workingDays:
        editData.workingDays && Array.isArray(editData.workingDays)
          ? editData.workingDays?.map((item) => {
              if (Days.filter((elm) => elm.id == item)[0]) {
                return Days.filter((elm) => elm.id == item)[0]["day"];
              }
            })
          : [],
    });
  ...
  const handleAddCapacity = (e) => {
    e.preventDefault();
    if (
      addCapacityData.workFromTime !== "" &&
      addCapacityData.workToTime !== "" &&
      addCapacityData.workingDays.length > 0
    ) {
      ...
      payload = {
        ...
        workingDays: addCapacityData.workingDays
          .filter((check) => check)
          .map((item) => {
            return Days.filter((elm) => elm.day == item)[0]["id"];
          }),
      };
        ...
        <div style={{ display: "flex", flexDirection: "row" }}>
          ...
          <div style={{ width: "25%" }}>
            ...
            <Select
              ...
              name="workingDays"
              value={
                addCapacityData.workingDays ? addCapacityData.workingDays : []
              }
              onChange={handleCapacity}
            ...
            >
              {Days.map((curr, index) => (
                <MenuItem key={index} value={curr.day}>
                  {curr.day}
                </MenuItem>
              ))}
            </Select>
              ...
              timeRange.map((item, index) => {
                ...
                if (index < 12) {
                  return (
                    <>
                      <div key={index}>
                        <label for={index}>{Object.keys(item)[0]}</label>
  ...
  return (
    <>
      ...
      {infoStatus && (
        <InfoDialog open={infoStatus} handleClose={() => setEditStatus(false)}>
          <CancelIcon
            style={{
              // top: 50,
              right: 10,
              ...
              <div style={{ width: "33%", display: "flex" }}>
                ...
                {Reg && (
                  <>
                    ...
                    >
                      {" "}
                      <Plus onClick={AddUser} style={{ cursor: "pointer" }} />
                      <X onClick={RemoveUser} style={{ cursor: "pointer" }} />
                    </div>
                  </>
                )}
              </div>
               ...
               {editData.labUserIds.map((curr)=>{
                ...
                >
                  {editData.idwiseName.map((curr, index) => (
                    <MenuItem key={index} value={curr.userName}>
                      {curr.userName}
                    </MenuItem>
                  ))}
                </Select>

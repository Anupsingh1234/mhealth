...
import message from "antd-message";
 const JoiningDetails = ({join,setJoin,bookingdetail1,cards}) => {
  ...
  const UpdateBooking = () => {
    const URL = `${urlPrefix}v1.0/updateCheckupBooking?action=${cancelData.action}&bookingId=${cancelData.bookingId}&text=${cancelData.text?cancelData.text:''}`;
    return axios
      .get(URL, {
        headers: {
    ...
    <div>
        {join&&(<InfoDialog open={join} onClose={setJoin}>
          ...
          <div style={{width:'600px',minHeight:'300px',maxHeight:'600px'}}>
                      ...
                      {bookingdetail1&&bookingdetail1.map((item,ind)=>{
                          ...
                          >
                            ...
                            <span style={{ fontSize: 12 }}>
                              {item.partnerLabName
                                ? item.partnerLabName
                                : "  -     "}<br/>{item.labAddress} ,{item.labCity}
                            </span>{" "}
                          </p>{" "}
                        </TableCell>
                        <TableCell>
              ...
              {cancelModal&&(<InfoDialog open={cancelModal} onClose={()=>setCancelModal(false)}>
              <CancelIcon
            style={{
              position: "absolute",
              top: 5,
          ...
          />
  <div style={{width:'400px',minHeight:'200px',maxHeight:'600px'}}>
  ...
  <textarea autofocus="autofocus"
                      ...
                      onChange={(e)=>setCancelData({...cancelData,text:e.target.value})}
                      name="sampleCollectionAddress"/>
                      </div>
                      <div style={{width:'80px',height:'10px',marginLeft:'75%'}} className="absolute  right-2" ><PrimaryButton onClick={UpdateBooking}  mini
                        className="w-[max-content] text-sm">Submit</PrimaryButton></div>
  </div>
              </InfoDialog>)}
          </div>
        </InfoDialog>)}

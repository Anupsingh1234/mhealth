...
const MarketPlace = ({
  ...
  const getDisAttached = (id) => {
    const adminurl = `${urlPrefix}v1.0/attachSubEvent?eventId=${eventId}&keyword=detach&subEventId=${id}`;
    //  setDependentValue(true);
    return axios
      .put(
        adminurl,
        ...
        {showUnsubscribeModal && (
          ...
          >
            <div className="event-unsubscribe-modal">
              <button
                onClick={() => {
                  getDisAttached(subEventDetail.id, subEventDetail.eventId);
                }}
              >
                Yes
              </button>
